...
#define kNCListerWorkingDir @"workingDir"
#define kNCListerNumberOfDirs @"numberOfDirs"
#define kNCListerNumberOfFiles @"numberOfFiles"
#define kNCListerSizeOfItems @"sizeOfItems"
#define kNCListerNumberOfSelectedDirs @"numberOfSelectedDirs"
#define kNCListerNumberOfSelectedFiles @"numberOfSelectedFiles"
#define kNCListerSizeOfSelectedItems @"sizeOfSelectedItems"
#define kNCListerPreferencesContext @"NCListerPreferencesContext"

static void* WorkingDirObservationContext = (void *)kNCListerWorkingDir;
static void* NumberOfDirsObservationContext = (void *)kNCListerNumberOfDirs;
static void* NumberOfFilesObservationContext = (void *)kNCListerNumberOfFiles;
static void* SizeOfItemsObservationContext = (void *)kNCListerSizeOfItems;
static void* NumberOfSelectedDirsObservationContext = (void *)kNCListerNumberOfSelectedDirs;
static void* NumberOfSelectedFilesObservationContext = (void *)kNCListerNumberOfSelectedFiles;
static void* SizeOfSelectedItemsObservationContext = (void *)kNCListerSizeOfSelectedItems;
...
+ (void)initialize {
    [self exposeBinding:kNCListerWorkingDir];
    [self exposeBinding:kNCListerNumberOfDirs];
    [self exposeBinding:kNCListerNumberOfFiles];     
    [self exposeBinding:kNCListerSizeOfItems];
    [self exposeBinding:kNCListerNumberOfSelectedDirs];
    [self exposeBinding:kNCListerNumberOfSelectedFiles];     
    [self exposeBinding:kNCListerSizeOfSelectedItems];
}
...
- (Class)valueClassForBinding:(NSString *)binding {
    if ([binding isEqualToString:kNCListerWorkingDir]) {
        return [NSString class];
    } else
    if ([binding isEqualToString:kNCListerNumberOfDirs]) {
        return [NSNumber class];
    } else
    if ([binding isEqualToString:kNCListerNumberOfFiles]) {
        return [NSNumber class];
    } else
    if ([binding isEqualToString:kNCListerSizeOfItems]) {
        return [NSNumber class];
    } else
    if ([binding isEqualToString:kNCListerNumberOfSelectedDirs]) {
        return [NSNumber class];
    } else
    if ([binding isEqualToString:kNCListerNumberOfSelectedFiles]) {
        return [NSNumber class];
    } else
    if ([binding isEqualToString:kNCListerSizeOfSelectedItems]) {
        return [NSNumber class];
    } else {
...
{	
	void* context = NULL;
    if ([bindingName isEqualToString:kNCListerWorkingDir]) {
		context = WorkingDirObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerNumberOfDirs]) {
		context = NumberOfDirsObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerNumberOfFiles]) {
		context = NumberOfFilesObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerSizeOfItems]) {
		context = SizeOfItemsObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerNumberOfSelectedDirs]) {
		context = NumberOfSelectedDirsObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerNumberOfSelectedFiles]) {
		context = NumberOfSelectedFilesObservationContext;
	} else
    if ([bindingName isEqualToString:kNCListerSizeOfSelectedItems]) {
		context = SizeOfSelectedItemsObservationContext;
	}

	if(context != NULL) {
		if ([m_binding_info objectForKey:bindingName] != nil) {
		...
		[m_binding_info setObject:obj forKey:bindingName];
...
- (void)unbind:bindingName {
    if ([bindingName isEqualToString:kNCListerWorkingDir]) {
		id obj = [self observedObjectForWorkingDir];
		NSString* kp = [self observedKeyPathForWorkingDir];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerWorkingDir];
    } else
    if ([bindingName isEqualToString:kNCListerNumberOfDirs]) {
		id obj = [self observedObjectForNumberOfDirs];
		NSString* kp = [self observedKeyPathForNumberOfDirs];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerNumberOfDirs];
    } else
    if ([bindingName isEqualToString:kNCListerNumberOfFiles]) {
		id obj = [self observedObjectForNumberOfFiles];
		NSString* kp = [self observedKeyPathForNumberOfFiles];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerNumberOfFiles];
    } else
    if ([bindingName isEqualToString:kNCListerSizeOfItems]) {
		id obj = [self observedObjectForSizeOfItems];
		NSString* kp = [self observedKeyPathForSizeOfItems];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerSizeOfItems];
    } else
    if ([bindingName isEqualToString:kNCListerNumberOfSelectedDirs]) {
		id obj = [self observedObjectForNumberOfSelectedDirs];
		NSString* kp = [self observedKeyPathForNumberOfSelectedDirs];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerNumberOfSelectedDirs];
    } else
    if ([bindingName isEqualToString:kNCListerNumberOfSelectedFiles]) {
		id obj = [self observedObjectForNumberOfSelectedFiles];
		NSString* kp = [self observedKeyPathForNumberOfSelectedFiles];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerNumberOfSelectedFiles];
    } else
    if ([bindingName isEqualToString:kNCListerSizeOfSelectedItems]) {
		id obj = [self observedObjectForSizeOfSelectedItems];
		NSString* kp = [self observedKeyPathForSizeOfSelectedItems];
		[obj removeObserver:self forKeyPath:kp];
		[m_binding_info removeObjectForKey:kNCListerSizeOfSelectedItems];
    } else {
					   ...
					   context:(void *)context
{
	...
	if (context == PreferencesObservationContext) {
		// LOG_DEBUG(@"preferences has changed");
		[self reloadTableSetup];
		return;
	}


	//BOOL allowsMultipleSelectionForAngle = NO;
    
    if (context == WorkingDirObservationContext)
    {
		// offset changed
		...
		if((value == NSNoSelectionMarker) 
			...
			|| (value == NSMultipleValuesMarker))
		{
			// placeholder
		}
		else
		{
			[self setValue:value forKey:kNCListerWorkingDir];
		    [self setNeedsDisplay:YES];
		}
		return;
    }
    
    if (context == NumberOfDirsObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerNumberOfDirs];
		    [self setNeedsDisplay:YES];
		}
		return;
    }

    if (context == NumberOfFilesObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerNumberOfFiles];
		    [self setNeedsDisplay:YES];
		}
		return;
    }

    if (context == SizeOfItemsObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerSizeOfItems];
		    [self setNeedsDisplay:YES];
		}
		return;
    }

    if (context == NumberOfSelectedDirsObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerNumberOfSelectedDirs];
		    [self setNeedsDisplay:YES];
		}
		return;
    }

    if (context == NumberOfSelectedFilesObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerNumberOfSelectedFiles];
		    [self setNeedsDisplay:YES];
		}
		return;
    }

    if (context == SizeOfSelectedItemsObservationContext)
    {
		// offset changed
		// if we got a NSNoSelectionMarker or NSNotApplicableMarker, or
		// if we got a NSMultipleValuesMarker and we don't allow multiple selections
		...
		{
			[self setValue:value forKey:kNCListerSizeOfSelectedItems];
		    [self setNeedsDisplay:YES];
		}
		return;
    }
...
- (NSDictionary *)infoForBinding:(NSString *)bindingName
{
	 ...
	 */
	NSDictionary *info = [m_binding_info objectForKey:bindingName];
	if (info == nil)
	{
...
-(id)observedObjectForWorkingDir { 
	return [[self infoForBinding:kNCListerWorkingDir] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForWorkingDir { 
	return [[self infoForBinding:kNCListerWorkingDir] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForNumberOfDirs { 
	return [[self infoForBinding:kNCListerNumberOfDirs] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForNumberOfDirs { 
	return [[self infoForBinding:kNCListerNumberOfDirs] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForNumberOfFiles { 
	return [[self infoForBinding:kNCListerNumberOfFiles] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForNumberOfFiles { 
	return [[self infoForBinding:kNCListerNumberOfFiles] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForSizeOfItems { 
	return [[self infoForBinding:kNCListerSizeOfItems] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForSizeOfItems { 
	return [[self infoForBinding:kNCListerSizeOfItems] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForNumberOfSelectedDirs { 
	return [[self infoForBinding:kNCListerNumberOfSelectedDirs] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForNumberOfSelectedDirs { 
	return [[self infoForBinding:kNCListerNumberOfSelectedDirs] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForNumberOfSelectedFiles { 
	return [[self infoForBinding:kNCListerNumberOfSelectedFiles] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForNumberOfSelectedFiles { 
	return [[self infoForBinding:kNCListerNumberOfSelectedFiles] objectForKey:NSObservedKeyPathKey];
}


-(id)observedObjectForSizeOfSelectedItems { 
	return [[self infoForBinding:kNCListerSizeOfSelectedItems] objectForKey:NSObservedObjectKey];
}

-(NSString*)observedKeyPathForSizeOfSelectedItems { 
	return [[self infoForBinding:kNCListerSizeOfSelectedItems] objectForKey:NSObservedKeyPathKey];
}
	...
	*/
	{
		...
		id obj = [self observedObjectForWorkingDir];
		if(obj != nil) {
			// LOG_DEBUG(@"reload setValue:'%@' forKeyPath", value);
			[obj setValue:value forKeyPath:[self observedKeyPathForWorkingDir]];
		}	
	}
	
	
	SEL sel = @selector(listerWillLoad:);
...
-(void)listerDataSourceFinishedLoading:(id<NCListerDataSource>)dataSource {
	...
	SEL sel = @selector(listerDidLoad:);
	if([m_delegate respondsToSelector:sel]) {
		[m_delegate performSelector:sel withObject:self];
	}
}
...
-(void)updateCounters {
	...
	[self setNumberOfDirs:number_of_dirs];
	[self setNumberOfFiles:number_of_files];
	[self setSizeOfItems:size_of_items];
	[self setNumberOfSelectedDirs:number_of_selected_dirs];
	[self setNumberOfSelectedFiles:number_of_selected_files];
	[self setSizeOfSelectedItems:size_of_selected_items];


	// update observed controller if set
	{
		id value = [NSNumber numberWithInt:number_of_dirs];
		id obj = [self observedObjectForNumberOfDirs];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForNumberOfDirs]];
		}	
	}
	{
		id value = [NSNumber numberWithInt:number_of_files];
		id obj = [self observedObjectForNumberOfFiles];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForNumberOfFiles]];
		}	
	}
	{
		id value = [NSNumber numberWithUnsignedLongLong:size_of_items];
		id obj = [self observedObjectForSizeOfItems];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForSizeOfItems]];
		}	
	}
	{
		id value = [NSNumber numberWithInt:number_of_selected_dirs];
		id obj = [self observedObjectForNumberOfSelectedDirs];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForNumberOfSelectedDirs]];
		}	
	}
	{
		id value = [NSNumber numberWithInt:number_of_selected_files];
		id obj = [self observedObjectForNumberOfSelectedFiles];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForNumberOfSelectedFiles]];
		}	
	}
	{
		id value = [NSNumber numberWithUnsignedLongLong:size_of_selected_items];
		id obj = [self observedObjectForSizeOfSelectedItems];
		if(obj != nil) {
			[obj setValue:value forKeyPath:[self observedKeyPathForSizeOfSelectedItems]];
		}	
	}
...
- (int)numberOfDirs {
	return m_number_of_dirs;
}

- (void)setNumberOfDirs:(int)n {
    if (m_number_of_dirs != n) {
        m_number_of_dirs = n;
    }
}
...
-(void)reloadTableSetup {
	...
	{
		NSString* name = font_and_size[font_index].font_name;
...
/*	{
	...
	if(m_lister_tableview == nil) {
		...
		NCScrollView* scrollView = [[NCScrollView alloc] initWithFrame:svrect];
		...
		if(0) {
			[hscroller adjustThemeForDictionary:[NCScroller darkTheme]];
			[vscroller adjustThemeForDictionary:[NCScroller darkTheme]];
		}
...
-(void)tabKeyPressed:(id)sender {
	// LOG_DEBUG(@"NCLister %s", _cmd);
	SEL sel = @selector(tabKeyPressed:);
	if([m_delegate respondsToSelector:sel]) {
		// LOG_DEBUG(@"NCLister %s yes", _cmd);
		[m_delegate performSelector:sel withObject:self];
	}
}
    ...
    row:(NSInteger)rowIndex
{
	if(rowIndex < [m_items count]) {
		...
		if(aTableColumn == m_tablecolumn_flags) {   
			...
			return [NSString stringWithFormat:@"%x", flags];
		}
...
-(void)showLeftContextMenu:(id)sender {
	NSMenu* menu = nil;
	SEL sel = @selector(leftMenu);
	if([m_delegate respondsToSelector:sel]) {
		menu = [m_delegate performSelector:sel];
	}
