Opcoders File Manager version 1

This version has been parked indefinitely. Mac OS X 10.7 is soon to come out and
I cannot use IBPlugins which will require quite some work to get rid of.
This version represents the best I could do without IBPlugins and using Xcode3.
In the next version I want to use Xcode4, but in order to do that I must get rid
of the IBPlugins. Thus version 1 ends here.
...
When it works then continue the big porting effort.


Will Xcode4 with Lion support IB-plugins?
https://devforums.apple.com/message/476316
https://devforums.apple.com/thread/85836?tstart=0

compile the entire project using Xcode4.
The IBPLugins aren't working. Is there a work around?
If not then rework all nib files to NOT use ibplugins.

remove NCCore.ibplugin dependency from InfoPanel.xib

remove NCCore.ibplugin dependency from CopySheet.xib

remove NCCore.ibplugin dependency from MoveSheet.xib

remove NCCore.ibplugin dependency from TestViews.xib

remove NCCore.ibplugin dependency from ListPanelTab.xib

remove NCCore.ibplugin dependency from ListPanel.xib

remove PSMTabBarControl.ibplugin dependency from ListView.xib
...
compile on 64 bit machine... requires that I purchase a new Mac


When Worker is ready, then NCWorker must be changed to Worker.


rework setuid bit permissions.. use AEWP instead as in Transmit
otherwise it will not be possible to get into the appstore
...
get rid of the scrollbars on Lion.
